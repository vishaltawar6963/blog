...
export const getUserBlogAction = (id) => async dispatch => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:5000/api/blog/user/${id}`)
...
export const addBlogAction = blogData => async (dispatch, getState) => {
    try {
        ...
        console.log("after");
        const { data } = await axios.post(`http://localhost:5000/api/blog`, blogData, config)
...
export const updateBlogAction = (blogData, id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:5000/api/blog/${id}`, { publish: blogData }, config)
...
export const deleteBlogAction = id => async (dispatch, getState) => {
    try {
        ...
        console.log(id);
        const { data } = await axios.delete(`http://localhost:5000/api/blog/${id}`, config)
